...
import {
  ...
  onGetBottles,
  onSetBottles,
  onGetBottlesFromFile,
} from "../../models/bottlesModels";

import { getImageSource } from "../common";
import {
  onSetImagesFromFolder,
  onDeleteImages,
  onGetImages,
...
const VinanticBO = () => {
  ...
  const onHandle = ({
    ...
    if (equals(label, "DELETE_BOTTLES_IN_BASE")) {
      setWarning(
        `La base de donnée est vide. ${deletedCount} bouteilles ont été supprimées`
      );
    } else if (equals(label, "GET_BOTTLES_FROM_BASE")) {
      setWarning(
        `La base de donnée est composé de ${gettedCount} bouteille(s)`
      );
    } else if (equals(label, "SET_BOTTLES_TO_BASE")) {
      setWarning(
        `La base de donnée a été mis à jour de ${settedCount} bouteille(s)`
      );
            ...
            <div className="my-10">
              {isNotEmpty(winesList) ? (
                <p className="font-serif text-lg">
                  Il y a {length(winesList)} bouteilles de vins référencées dans
                  le fichier
                </p>
              ) : (
                <p className="font-serif text-lg">
                  Aucune bouteille à envoyer en base ! Importez depuis un
                  fichier.
                </p>
              )}
              {isNotEmpty(warning) && (
                <p className="font-serif text-red-300 text-lg">{warning}</p>
              )}
              {isNotEmpty(imagesList) && (
                <p className="font-serif text-red-300 text-lg">{`Il y a ${length(
                  imagesList
                )} images en base`}</p>
              )}
            </div>
